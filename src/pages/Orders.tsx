...
export default function Orders() {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'Shipped': return 'bg-emerald-900/20 text-emerald-500 border-emerald-500/20';
  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex justify-between items-end border-b border-slate-200 dark:border-white/5 pb-6">
        <div>
          <h2 className="font-serif text-3xl text-slate-900 dark:text-white">Order Registry</h2>
              ...
              <tbody className="text-sm divide-y divide-white/[0.05]">
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="py-5 text-emerald-400 font-medium">${order.totalAmount.toFixed(2)}</td>
                    <td className="py-5">
        ...
        <div className="fixed inset-0 bg-slate-50 dark:bg-[#0a0b0d]/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#07080a] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-200 dark:border-white/5 flex justify-between items-center">
              <h3 className="font-serif text-xl text-slate-900 dark:text-white">New Order</h3>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-5 relative">
                <div className="col-span-2">
                  ...
                  }} className="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-emerald-500/50 transition-colors" />
              ...
              <div className="bg-emerald-900/10 border border-emerald-500/20 p-4 rounded-lg flex items-start mt-6">
                <MessageSquare className="w-4 h-4 text-emerald-500 mt-0.5 mr-3 flex-shrink-0" />
                <p className="text-[11px] text-emerald-400">A confirmation message will be sent automatically via WhatsApp upon creation.</p>
              </div>
      ...
      {editingOrder && (
        <div className="fixed inset-0 bg-slate-50 dark:bg-[#0a0b0d]/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#07080a] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-200 dark:border-white/5 flex justify-between items-center">
              <h3 className="font-serif text-xl text-slate-900 dark:text-white">Update Order <span className="text-slate-500 text-base font-mono">{editingOrder.id}</span></h3>
            ...
            <form onSubmit={handleUpdateStatus} className="p-6 space-y-6">
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2.5 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-emerald-500/50 transition-colors"
                >
              ...
              <label className="flex items-center space-x-4 p-4 border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02] rounded-xl cursor-pointer hover:bg-white/[0.04] transition-colors">
                <div className={cn("w-5 h-5 rounded border flex items-center justify-center transition-colors flex-shrink-0", sendUpdateMsg ? "bg-emerald-500 border-emerald-500" : "border-slate-300 dark:border-white/20 bg-transparent")}>
